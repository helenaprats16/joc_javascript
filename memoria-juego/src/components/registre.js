...
function registre() {
  ...
  // Configurar validació reactiva
  setupRegistreValidation(div);
  
  return div;
}

function setupRegistreValidation(container) {
  const formElement = container.querySelector('#registreForm');
  const emailInput = container.querySelector('#email');
  const passwordInput = container.querySelector('#contrasenya');
  const confirmPasswordInput = container.querySelector('#contrasenya-repeat');
  const submitBtn = container.querySelector('#boto');
  ...
  // Subscriure's als canvis de confirmació de contrasenya
